Z:/OMG_share/{fn =~ /2160p|4K|4k|UHD/ ? 'Movies.4K' : 'Movies'}
/
...
{aco.match(/Atmos/)}).join(' ')}{" $channels"}
{" $VC"}{def dn = call{(file.parentFile.name).match(/(?:\-\w+(?=\[\w+\])|\-\w+$)/)};
